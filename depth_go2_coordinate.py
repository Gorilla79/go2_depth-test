...
try:
    ...
    while True:
        ...
        for coord in coordinates:
            print(f"[Coordinate] ID: {coord[0]}, X: {coord[1]}, Y: {coord[2]:.2f}")

        # Create a blank image for the coordinate map
        map_image = np.zeros((500, 500, 3), dtype=np.uint8)

        # Draw the robot at the center
        cv2.circle(map_image, (250, 250), 5, (0, 0, 0), -1)
        cv2.putText(map_image, "ROBOT", (255, 245), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        # Draw x and y axes
        cv2.line(map_image, (0, 250), (500, 250), (255, 255, 255), 1)
        cv2.line(map_image, (250, 0), (250, 500), (255, 255, 255), 1)

        # Draw the detected humans on the map
        for human_id, x, y in coordinates:
            map_x = int(250 + x)
            map_y = int(250 - y * 100)  # Scale the depth for better visualization
            cv2.circle(map_image, (map_x, map_y), 5, (0, 255, 255), -1)
            cv2.putText(map_image, f"ID:{human_id}", (map_x + 5, map_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
            cv2.putText(map_image, f"({x}, {y:.2f})", (map_x + 5, map_y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
